<style>
  ...
  .dash-toggle{ position:fixed; left:12px; top:12px; z-index:1041; background:#1E40FF; color:#fff; border:none; padding:.45rem .6rem; border-radius:.5rem; box-shadow:0 .5rem 1rem rgba(0,0,0,.25); display:none; }
  .dash-overlay{ position:fixed; inset:0; background:rgba(0,0,0,.45); z-index:1040; display:none; }
  .dash-overlay.show{ display:block; }
  /* Ensure full height */
  .dash-sidebar{ min-height:calc(100dvh - 2rem); transition: transform .25s ease, width .25s ease; will-change: transform; z-index:1042; background-color: #1E40FF; }
  /* Close button hidden on desktop by default */
  .dash-close{ display:none; }
  /* Mobile drawer (hidden by default) */
  @media (max-width: 991.98px){
    .dash-toggle{ display:inline-block; }
    .dash-sidebar{ position:fixed; top:1rem; left:1rem; transform:translateX(-110%); width:280px; background-color: #1E40FF; }
  ...
  .dash-logo .circle{ width:96px; height:96px; border-radius:50%; background:#1E40FF; display:grid; place-items:center; box-shadow:inset 0 -8px 14px rgba(0,0,0,.25); }
  .dash-menu{ list-style:none; padding:0; margin:1rem 0 0; display:grid; gap:.5rem; }
  .dash-menu a{ display:flex; align-items:center; gap:.6rem; padding:.65rem .9rem; border-radius:.6rem; color:#E5E7EB; text-decoration:none; font-weight:700; background:transparent; }
  .dash-menu a:hover, .dash-menu a.active{ background:#1E3AFA; color:#fff; }
